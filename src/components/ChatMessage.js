  ...
  return (
    <div className="space-y-8">
      ...
      {(selectedSubreddits.length > 0 || searchQueries.length > 0 || isProcessing) && (
        <div className="p-6 bg-[#fbfbfd] rounded-[24px] border border-[#e5e5e7] space-y-4 animate-in fade-in relative overflow-hidden shadow-sm">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <span className={`text-[10px] font-black uppercase tracking-[0.2em] ${isProcessing ? 'text-[#ff3b30]' : 'text-[#86868b]'}`}>
                {isProcessing ? 'Optimizing Extraction' : 'Extraction Strategy'}
              </span>
            </div>
            {totalScanned > 0 && (
              <span className="text-[10px] text-[#1d1d1f] font-bold bg-[#f2f2f2] px-3 py-1 rounded-full border border-[#e5e5e7] tracking-widest uppercase">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {searchQueries.length > 0 && (
              <div className="space-y-2">
                <span className="text-[9px] font-black text-gray-400 uppercase tracking-widest">Logic Filters</span>
                <div className="flex flex-wrap gap-1.5">
                  {searchQueries.map((q, idx) => (
                    <span key={idx} className="px-2.5 py-1 bg-white border border-[#e5e5e7] rounded-lg text-[10px] font-medium text-[#1d1d1f] font-mono shadow-sm">
                      {q}
                    </span>
                  ))}
                </div>
            ...
            {selectedSubreddits.length > 0 && (
              <div className="space-y-2">
                <span className="text-[9px] font-black text-gray-400 uppercase tracking-widest">Sources</span>
                <div className="flex flex-wrap gap-1.5">
                  {selectedSubreddits.map(s => (
                    <span key={s} className="px-2.5 py-1 rounded-lg bg-white text-[#1d1d1f] text-[10px] font-bold border border-[#e5e5e7] shadow-sm">r/{s}</span>
                  ))}
                </div>
              </div>
            )}
      ...
      {leads.length > 0 ? (
        <div className="space-y-6 animate-in slide-in-from-bottom-8 duration-700">
          <div className="flex justify-between items-center px-2">
            <div className="flex items-center gap-2">
              ...
              <h4 className="text-[10px] font-black text-gray-400 uppercase tracking-[0.3em]">Qualified Prospects</h4>
            </div>
            {onExport && (
          ...
          <div className="space-y-4">
            {leads.map((lead, idx) => {
              ...
              return (
                <div key={idx} className="bg-white rounded-[24px] border border-[#e5e5e7] p-6 hover:shadow-xl transition-all duration-500 group shadow-sm">
                  <div className="flex flex-col md:flex-row gap-6">
                    {/* Left: Lead Profile */}
                    <div className="md:w-64 space-y-4 shrink-0">
                      <div className="flex items-center gap-3">
                        <div className="w-12 h-12 rounded-xl bg-[#f5f5f7] flex items-center justify-center text-xl shadow-sm group-hover:scale-110 transition-transform duration-500">
                          {type.icon}
                        </div>
                        <div>
                          ...
                          <div className="font-bold text-[#1d1d1f] flex items-center gap-1.5">
                            u/{lead.author}
                            <a href={`https://reddit.com/u/${lead.author}`} target="_blank" rel="noreferrer" className="text-gray-300 hover:text-[#ff3b30] transition-colors">
                      ...
                      <div className="p-4 bg-[#f5f5f7] rounded-2xl border border-[#e5e5e7]">
                        <div className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-2 flex justify-between items-center">
                          <span>Intent Score</span>
                          <span className={scoreColor}>{score}/10</span>
                        </div>
                        <div className="h-1.5 bg-white rounded-full overflow-hidden border border-[#e5e5e7]">
                    ...
                    <div className="flex-1 space-y-4">
                      <div className="flex items-center justify-between">
                        <span className="text-[10px] font-bold px-3 py-1 bg-[#f5f5f7] border border-[#e5e5e7] rounded-full text-[#86868b] uppercase tracking-widest">r/{lead.subreddit}</span>
                      </div>

                      <h3 className="font-bold text-lg text-[#1d1d1f] leading-tight hover:text-[#ff3b30] transition-colors cursor-pointer pr-12">
                        <a href={lead.link} target="_blank" rel="noreferrer">{lead.title}</a>
                      </h3>

                      <div className="flex flex-wrap gap-2 mt-4">
                        {lead.emails?.map((e, idx) => (
                          <div key={idx} className="flex items-center gap-2 px-3 py-1.5 bg-[#fbfbfd] border border-[#e5e5e7] text-[#1d1d1f] rounded-xl text-xs font-medium cursor-pointer hover:bg-[#ff3b30]/10 hover:border-[#ff3b30]/20 hover:text-[#ff3b30] transition-all group/badge" onClick={() => copy(e)}>
                    ...
                    <div className="flex flex-row md:flex-col gap-3 shrink-0">
                      <button
                        ...
                        className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all shadow-sm ${savedLeads.has(lead.id)
                            ...
                            : 'bg-[#f5f5f7] border border-[#e5e5e7] text-[#86868b] hover:bg-white hover:text-[#1d1d1f] hover:border-[#ff3b30]'
                          }`}
                  ...
                  <div className="mt-8 space-y-3">
                    {lead.body && (
                      <div className="p-5 bg-[#fbfbfd] rounded-2xl border border-[#e5e5e7]">
                        ...
                        <p className="text-sm text-[#1d1d1f] font-medium leading-relaxed italic pr-4">"{lead.body}"</p>
                      </div>
                    )}
                    ...
                    {lead.suggestedReply && (
                      <div className="relative p-5 bg-[#ff3b30]/5 border border-[#ff3b30]/10 rounded-2xl group/reply cursor-pointer hover:bg-[#ff3b30]/10 transition-all" onClick={() => copy(lead.suggestedReply)}>
                        ...
                        <p className="text-sm text-[#1d1d1f] leading-relaxed font-medium">"{lead.suggestedReply}"</p>
        ...
        isCompleted && !error && (
          <div className="p-16 bg-[#fbfbfd] border border-dashed border-[#e5e5e7] rounded-[40px] text-center space-y-6 animate-in zoom-in-95 duration-700">
            <div className="w-20 h-20 bg-white border border-[#e5e5e7] rounded-3xl flex items-center justify-center mx-auto shadow-sm">
              <span className="material-symbols-outlined text-[#86868b] text-[40px]">manage_search</span>
            </div>
            <div className="space-y-2">
              <h3 className="text-lg font-bold text-[#1d1d1f] tracking-tight">Zero High-Intent Signals Found</h3>
              <p className="text-sm text-[#86868b] max-w-[400px] mx-auto leading-relaxed font-medium">
      ...
      {insights && isCompleted && (
        <div className="p-8 bg-white rounded-[32px] border border-[#e5e5e7] space-y-6 animate-in fade-in duration-1000 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.3em] text-[#86868b]">Strategic Intelligence</span>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            {insights.topPainPoints?.length > 0 && (
              <div className="space-y-4">
                <span className="text-[10px] font-black text-[#86868b] uppercase tracking-widest flex items-center gap-2">
                  <span className="w-1 h-1 bg-[#ff3b30] rounded-full" /> Market Pain Points
                </span>
                <ul className="space-y-3">
                  {insights.topPainPoints.map((p, idx) => (
                    <li key={idx} className="text-sm font-medium text-[#1d1d1f] flex items-start gap-3">
      ...
      {toast && (
        <div className="fixed bottom-12 right-6 z-[100] px-6 py-3 bg-[#1d1d1f] text-white text-xs font-bold rounded-2xl shadow-2xl animate-in border border-white/10">
          <div className="flex items-center gap-2">
